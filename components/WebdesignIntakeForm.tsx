...
const projectOptions: Array<{ value: ProjectTypeValue; title: string; body: string }> = [
  { value: 'landing', title: 'Landingpage', body: 'Eine kompakte Seite fuer ein Angebot, eine Dienstleistung oder eine klare Anfrage.' },
...
const goalOptions: Array<{ value: GoalValue; label: string }> = [
  { value: 'anfragen', label: 'Mehr Anfragen ueber die Website' },
  { value: 'vertrauen', label: 'Serioeser und klarer wirken' },
...
const timelineOptions: Array<{ value: TimelineValue; label: string; body: string }> = [
  { value: 'soon', label: 'Moeglichst bald', body: 'Die Website wird kurzfristig gebraucht.' },
  { value: 'month', label: 'In 1-2 Monaten', body: 'Es gibt schon Bedarf, aber noch etwas Spielraum.' },
  { value: 'quarter', label: 'In den naechsten 3 Monaten', body: 'Das Projekt soll geplant und sauber vorbereitet werden.' },
  { value: 'open', label: 'Zeitlich offen', body: 'Die Richtung ist wichtig, der Starttermin ist noch flexibel.' },
];

const steps = [
  { id: '01', label: 'Ausgangslage', title: 'Worum geht es und was existiert schon?' },
  { id: '02', label: 'Umfang', title: 'Welche Art Website soll entstehen?' },
  { id: '03', label: 'Funktionen', title: 'Was muss die Website konkret koennen?' },
  { id: '04', label: 'Kontakt', title: 'Wie soll ich darauf antworten?' },
] as const;

const inputBaseClassName =
  'w-full rounded-[1rem] border border-white/80 bg-[linear-gradient(180deg,rgba(255,255,255,0.92),rgba(246,249,253,0.76))] px-4 py-3 text-[0.98rem] text-slate-800 shadow-[inset_0_1px_0_rgba(255,255,255,0.82)] outline-none transition-all duration-200 placeholder:text-slate-400 focus:border-blue-300 focus:ring-2 focus:ring-blue-200';
...
const WebdesignIntakeForm: React.FC = () => {
  ...
  const validateStep = (stepIndex: number) => {
    ...
    if (stepIndex === 0) {
      if (!formData.businessName.trim()) nextErrors.businessName = 'Bitte kurz Unternehmen oder Projekt benennen.';
      if (!formData.companySummary.trim()) nextErrors.companySummary = 'Bitte kurz erklaeren, worum es geht.';
      if (!formData.existingSite) nextErrors.existingSite = 'Bitte angeben, ob bereits eine Website existiert.';
    }

    if (stepIndex === 1) {
      if (!formData.projectType) nextErrors.projectType = 'Bitte den geplanten Website-Typ waehlen.';
      if (!formData.primaryGoal) nextErrors.primaryGoal = 'Bitte das wichtigste Ziel angeben.';
    }

    if (stepIndex === 2) {
      if (!formData.timeline) nextErrors.timeline = 'Bitte ein ungefaehres Zeitfenster waehlen.';
    }

    if (stepIndex === 3) {
      if (!formData.contactName.trim()) nextErrors.contactName = 'Bitte einen Namen fuer die Rueckmeldung angeben.';
      if (!formData.email.trim()) {
        nextErrors.email = 'Eine E-Mail-Adresse ist noetig, damit ich antworten kann.';
  ...
  return (
    <section id="project-intake" className="relative scroll-mt-28 py-12 sm:py-16 lg:py-20">
      ...
      <div className="relative z-10 grid gap-8 xl:grid-cols-[minmax(0,0.9fr)_minmax(0,1.1fr)] xl:gap-10">
        <motion.div
        ...
        >
          ...
          <div className={`space-y-4 rounded-[1.2rem] p-5 sm:p-6 ${themeClasses.webPanelSoft}`}>
          ...
          <div className={`rounded-[1.2rem] p-5 sm:p-6 ${themeClasses.webCard}`}>
          ...
          <div className="relative z-10 space-y-5">
            ...
            <AnimatePresence mode="wait">
              ...
              >
                  ...
                  <>
                    <div>
                      ...
                      <input
                        ...
                        placeholder="z. B. Praxis, Handwerksbetrieb oder lokales Unternehmen"
                      />
                    ...
                    <div>
                      <label className="mb-2 block text-sm font-semibold text-slate-700" htmlFor="companySummary">
                        Worum geht es und was soll die Website vermitteln?
                      </label>
                      <textarea
                        id="companySummary"
                        ...
                        placeholder="Kurz in normalen Worten: Was bieten Sie an? Fuer wen? Was soll ein Besucher schnell verstehen?"
                      />
                      {errors.companySummary && <p className="mt-2 text-sm text-rose-600">{errors.companySummary}</p>}
                    </div>

                    <fieldset className="space-y-3">
                      <legend className="text-sm font-semibold text-slate-700">Existiert schon eine Website?</legend>
                      <div className="grid gap-3 sm:grid-cols-3">
                        {[
                          { value: 'none', label: 'Noch keine', body: 'Es soll neu gestartet werden.' },
                          { value: 'live', label: 'Ja, sie ist live', body: 'Es gibt bereits eine laufende Website.' },
                          ...
                          return (
                            <label
                              key={option.value}
                              className={`cursor-pointer rounded-[1rem] border px-4 py-4 transition-all duration-200 ${checked ? 'border-blue-300 bg-blue-50/80 shadow-[0_10px_22px_rgba(59,130,246,0.10)]' : 'border-white/80 bg-white/66 hover:border-blue-200'}`}
                            >
                              <input
                                type="radio"
                                name="existingSite"
                    ...
                    {(formData.existingSite === 'live' || formData.existingSite === 'relaunch') && (
                      <div>
                        <label className="mb-2 block text-sm font-semibold text-slate-700" htmlFor="existingUrl">
                          Falls vorhanden: aktuelle Website-URL
                        </label>
                        <input
                          id="existingUrl"
                          type="url"
                ...
                {currentStep === 1 && (
                  <>
                    <fieldset className="space-y-3">
                      <legend className="text-sm font-semibold text-slate-700">Was wird gebraucht?</legend>
                    ...
                    <fieldset className="space-y-3">
                      <legend className="text-sm font-semibold text-slate-700">Was ist im Kern das wichtigste Ziel?</legend>
                  ...
                  <>
                    <fieldset className="space-y-3">
                      ...
                      <p className="text-sm leading-relaxed text-slate-600">
                        Nur ankreuzen, was wirklich eine Rolle spielt. Wenn etwas noch unklar ist, einfach frei lassen.
                      </p>
                      <div className="grid gap-3 sm:grid-cols-2">
                    ...
                    <fieldset className="space-y-3">
                      <legend className="text-sm font-semibold text-slate-700">Wann soll das Thema idealerweise starten?</legend>
                      <div className="grid gap-3 sm:grid-cols-2">
                    ...
                    <div>
                      <label className="mb-2 block text-sm font-semibold text-slate-700" htmlFor="styleNotes">
                        Optional: Gibt es noch Hinweise oder spezielle Wuensche?
                      </label>
                      <textarea
                        id="styleNotes"
                        ...
                        className={textareaClassName}
                        placeholder="z. B. moderner wirken, bestehende Farben behalten, mobil besonders wichtig, Terminbuchung spaeter moeglich ..."
                      />
                    </div>
                  </>
                )}
                ...
                {currentStep === 3 && (
                  <>
                    <div className={`rounded-[1rem] p-4 sm:p-5 ${themeClasses.webCard}`}>
                      <p className={themeClasses.webEyebrow}>Ihre Angaben auf einen Blick</p>
            ...
            <div className="flex flex-col gap-3 border-t border-blue-100 pt-4 sm:flex-row sm:items-center sm:justify-between">
              ...
              >
                ...
                <span className={`${themeClasses.webMeta} font-bold text-slate-800`}>Zurueck</span>
              </button>

              {currentStep < steps.length - 1 ? (
                <button type="button" onClick={goNext} className={themeClasses.webButtonPrimary}>
                  <span className={`${themeClasses.webMeta} font-bold text-white`}>Weiter zum naechsten Schritt</span>
